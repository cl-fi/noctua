...
import { NOCTUA_TOOLS } from './tools.js';
...
export class GeminiBrain {
    ai;
    toolHandler;
    chatSessions = new Map(); // per chatId
...
- Monitoring paused: ${data.rule.paused}`;
        ...
        try {
            ...
            while (response.functionCalls && response.functionCalls.length > 0 && maxRounds > 0) {
                ...
                for (const fc of response.functionCalls) {
                    const result = await this.toolHandler.handle(fc.name, fc.args || {});
     ...
     */
    async calibrateHF(volatility, position) {
...
- Debts: ${position.debts.map(d => `${d.symbol}: ${d.amount.toFixed(4)} ($${d.valueUsd.toFixed(2)})`).join(', ')}`;
        try {
            ...
            if (!jsonMatch)
                throw new Error('No valid JSON in calibration response');
