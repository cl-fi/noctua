...
export class NoctuaDaemon {
    ...
    constructor(config, rule) {
        ...
        const toolHandler = new ToolHandler(this.naviClient, this.unwindEngine, this.walrusLogger, () => this.getState(), (rule) => this.updateRule(rule));
    ...
    async check() {
        if (this.isProcessing || this.state.rule.paused)
            return;
        ...
        try {
            ...
            if (this.state.hfHistory.length > 20)
                this.state.hfHistory.shift();
