...
program
    ...
    if (opts.detach) {
        ...
        const args = ['--daemon'];
        if (opts.trigger)
            args.push('--trigger', String(opts.trigger));
        if (opts.target)
    ...
    const rule = {
        ...
        strategy: opts.strategy || 'collateral_swap',
        paused: false,
    };
...
program
    ...
    try {
        const state = JSON.parse(fs.readFileSync(stateFile, 'utf-8'));
        if (opts.trigger)
            state.rule.triggerHF = opts.trigger;
        if (opts.target)
            state.rule.targetHF = opts.target;
        if (opts.strategy)
            state.rule.strategy = opts.strategy;
        if (opts.pause)
            state.rule.paused = true;
        if (opts.resume)
            state.rule.paused = false;
...
function colorHF(hf) {
    if (hf <= 1.1)
        return chalk.bgRed.white(` ${hf.toFixed(4)} `);
    if (hf <= 1.3)
        return chalk.red(hf.toFixed(4));
    if (hf <= 1.5)
        return chalk.yellow(hf.toFixed(4));
    if (hf <= 2.0)
        return chalk.green(hf.toFixed(4));
